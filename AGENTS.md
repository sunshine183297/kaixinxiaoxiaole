...
## Project Overview
- **猫猫消消乐** — a cat-themed match-3 game built with Cocos Creator 3.8.6 and TypeScript.
- Target platforms: **WeChat Mini Game** and Web.
- Source code lives under `assets/Script/`.
- Scenes: `assets/Scene/Login.scene`, `assets/Scene/Level.scene`, and `assets/Scene/Game.scene`.
- Level flow: `LoginController` preloads Level scene -> `LevelSelectController` -> `GameController`.
- Theme: 6 cat breeds (橘猫/英短蓝猫/布偶猫/暹罗猫/三花猫/黑猫) + 1 special golden cat piece.
- WeChat build template lives under `build-templates/wechatgame/`.
...
- In Creator: `Project -> Build`, select **WeChat Mini Game** platform.
- Fill in AppID from `build-templates/wechatgame/project.config.json`.
- Open the build output folder in **WeChat DevTools** for debugging and uploading.
...
- Cat-themed assets should use descriptive Chinese/English names.
...
- Use `WechatCompat` utility (`assets/Script/Utils/WechatCompat.ts`) for platform-specific API calls.
- All WeChat APIs are safely guarded; they no-op on non-WeChat platforms.
- Use `sys.localStorage` (wrapped by `WechatCompat.safeGetItem`/`safeSetItem` for extra safety) for persistence.
...
- For WeChat testing, build in Creator then open output in WeChat DevTools.
